...
{
	public partial class DocumentSCM : System.Web.UI.Page
	{
		...
		protected void btnCreate_Click(object sender, EventArgs e)
		{
			try
			{
				...
				else
				{
					...
					_dataManager.MakeInParam("@PoNo", SqlDbType.NVarChar, 500, ""),
					_dataManager.MakeInParam("@SupplierName", SqlDbType.NVarChar, 500, ""),
		...
		protected Boolean UpdateDocumentInformation(String sAction)
		{
			...
			try
			{
				...
				SqlParameter[] parameters = new SqlParameter[26]
				{
					...
					_dataManager.MakeInParam("@CategoryID", SqlDbType.NVarChar, 500, hfCategoryId.Value),
					_dataManager.MakeInParam("@Remarks", SqlDbType.NVarChar, 500, txtRemarks.Text),
					_dataManager.MakeInParam("@Comments", SqlDbType.NVarChar, 500, txtRemarksBoss.Text),
			...
			try
			{
				foreach (GridViewRow gvRow in gvDetails.Rows)
				{
					if (((CheckBox)gvRow.FindControl("gvCbSelect")).Checked)
					{
						...
						mrrId = HttpUtility.HtmlDecode((phfType.Value.ToString() == "MR" ? gvRow.Cells[1].Text : ""));
						...
						else if (phfType.Value.ToString() == "MR")
						{
							_dataManager = new DataManager();
							SqlParameter[] parameters = new SqlParameter[10]
							{
								_dataManager.MakeInParam("@DocumentId", SqlDbType.NVarChar, 500, hfDocumentId.Value),
								_dataManager.MakeInParam("@MrrId", SqlDbType.NVarChar, 500,mrrId),
								_dataManager.MakeInParam("@PoId", SqlDbType.NVarChar, 500,"0"),
								_dataManager.MakeInParam("@PoNo", SqlDbType.NVarChar, 500,""),
								...
								_dataManager.MakeInParam("@ChallanNo", SqlDbType.NVarChar, 500,sChallanNo),
								_dataManager.MakeInParam("@MrrAmt", SqlDbType.Float, 500,sMrrAmt),
								_dataManager.MakeInParam("@UserId", SqlDbType.NVarChar, 500, _user.GetCookie(CookieKey.UserId.ToString())),
								_dataManager.MakeInParam("@Action", SqlDbType.NVarChar, 500, "INSERT")
							};
		...
		protected void btnUpload_Click(object sender, EventArgs e)
		 {
			...
			{
				lblMessage.Text = "Only PDF and image files allowed.";
				return;
			}
			...
			RegisterAsyncTask(new PageAsyncTask(ProcessDocument));
		}
		...
		private async Task ProcessDocument()
		{
			try
			{
				...
				string ext = Session["FileExt"] as string;
				...
				string clean = CleanJson(result);

				var dataList =
					Newtonsoft.Json.JsonConvert.DeserializeObject<List<InvoiceResult>>(clean);

				if (dataList == null || dataList.Count == 0)
				{
					lblMessage.Text = "No data extracted.";
					return;
				}
				...
				// ================= UI =================
				txtBillNo.Text = data.bill_no == "NAN" ? "NA" : data.bill_no;
				txtBillAmount.Text = data.bill_amount == "NAN" ? "NA" : data.bill_amount;
				txtDiscountAmount.Text = data.discount_amount == "NAN" ? "0" : data.discount_amount;
				txtFinalAmount.Text = data.final_amount == "NAN" ? "NA" : data.final_amount;
				txtBillDate.Text = data.txtBillDate == "NAN" ? "NA" : data.txtBillDate;

				txtVatAmount.Text = data.vat_details?.vat_amount == "NAN" ? "NA" : data.vat_details?.vat_amount;
				txtVATChallanNo.Text = data.vat_details?.vat_number == "NAN" ? "NA" : data.vat_details?.vat_number;
				txtVATDate.Text = data.vat_details?.vat_date == "NAN" ? "NA" : data.vat_details?.vat_date;

				txtRemarks.Text = data.Documents_Received == "NAN" ? "" : data.Documents_Received;

				lblMessage.Text = "Extraction completed successfully.";
			}
			catch (Exception ex)
			{
		...
		private async Task<string> ProcessPdf(string pdfPath)
		{
			byte[] fileBytes = File.ReadAllBytes(pdfPath);
			string base64 = Convert.ToBase64String(fileBytes);

			var googleAI = new GoogleAi("AIzaSyAjdTF-u5kZAEJyAG5G65mRvDx-AOcO5GM");
		...
		private async Task<string> ProcessImage(string imagePath)
		{
			...
			var googleAI = new GoogleAi("AIzaSyCyjQgmNPXnY14LGmFWq8q4tN84aANJl30");
			var model = googleAI.CreateGenerativeModel("gemini-3-flash-preview");
...
[
  {
    ...
    ""Documents_Received"": ""Identify document type (Invoice, VAT, Receipt). If multiple pages exist, combine like 'Invoice & VAT'"",
 
    ""vendor_identity"": ""Name of company issuing the bill"",
 
    ""total_amount_due"": ""Final total amount in numeric format only"",
 
    ""bill_no"": ""Invoice or bill number"",
 
    ""txtBillDate"": ""Invoice date (format DD-MM-YYYY if possible)"",
 
    ""bill_amount"": ""Subtotal before discounts or tax"",
 
    ""discount_amount"": ""Total discount applied (if none, return 0)"",
 
    ""final_amount"": ""Final payable amount after all adjustments"",
    ...
    ""vat_details"": {
      ""vat_receipt_exists"": true,
      ""vat_number"": ""VAT/Mushok/Challan number if available"",
      ""vat_date"": ""VAT document date"",
      ""vat_amount"": ""VAT amount only"",
      ""vat_total_with_tax"": ""Total including VAT""
    }
  }
]
...
";
		...
		public class InvoiceResult
		{
			...
			public double extraction_confidence { get; set; }

			// New Field Added Here
			public string Documents_Received { get; set; }

			public string vendor_identity { get; set; }
			public string total_amount_due { get; set; }
			public string bill_no { get; set; }
			public string txtBillDate { get; set; }
			public string bill_amount { get; set; }
			public string discount_amount { get; set; }
			public string final_amount { get; set; }

			public List<LineItem> line_items { get; set; }
			public VatDetails vat_details { get; set; }
		}

		public class VatDetails
		{
			public bool vat_receipt_exists { get; set; }
			public string vat_number { get; set; }
			public string vat_date { get; set; }
			public string vat_amount { get; set; }
			public string vat_total_with_tax { get; set; }
		}
